...
"""
import sys
sys.path.append('/home/jiarui/models/research/')
import math
import os.path
import time
import numpy as np
...
from deeplab import model_confusion
from visulaization.utils import segmentation_dataset
from visulaization.utils import input_generator_wolabel
from visulaization.utils import save_annotation
...
flags.DEFINE_string('image_side', 'left',
                    'Which image side of the dataset used for visualizing results')
...
def _process_batch(sess, original_images, semantic_predictions, image_names,
  ...
  for i in range(num_image):
    ...
    (image_filename,_)=os.path.splitext(os.path.basename(image_names[i]))
    ...
    # Save prediction.
    #print(semantic_prediction)
    #print(crop_semantic_prediction)
    #print(np.shape(crop_semantic_prediction))
    ...
    #filename = '%s/%s_confidence.txt' % (save_dir, image_filename)
    #np.savetxt(filename, crop_semantic_prediction)
    save_annotation.save_annotation(
...
def main(unused_argv):
  ...
  dataset = segmentation_dataset.get_dataset(
      FLAGS.dataset, FLAGS.vis_split, FLAGS.image_side, dataset_dir=FLAGS.dataset_dir)
  train_id_to_eval_id = None
  ...
  g = tf.Graph()
  with g.as_default():
    samples = input_generator_wolabel.get(dataset,
                                  FLAGS.vis_crop_size,
    ...
    if tuple(FLAGS.eval_scales) == (1.0,):
      ...
      predictions, scales_to_logits, logits = model_confusion.predict_labels(
          samples[common.IMAGE],
          model_options=model_options,
          image_pyramid=FLAGS.image_pyramid)
    else:
      tf.logging.info('Performing multi-scale test.')
      predictions = model_confusion.predict_labels_multi_scale(
          samples[common.IMAGE],
          model_options=model_options,
    ...
    while (FLAGS.max_number_of_iterations <= 0 or
      ...
      with sv.managed_session(FLAGS.master,
        ...
        for batch in range(num_batches):
         ...
         # print(sess.run(scales_to_logits))
         # print(sess.run(logits))
         # logit_array = sess.run(logits)
         # print(np.shape(logit_array))
