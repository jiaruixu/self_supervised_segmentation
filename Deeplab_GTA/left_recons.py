...
_FOLDERS_MAP = {
    'right': '/home/jiarui/git/Deeplab_GTA/rightvis/segmentation_results',
    'right_fullseg': '/home/jiarui/git/Deeplab_GTA/rightvis_fullseg/segmentation_results',
    'left': '/home/jiarui/git/Deeplab_GTA/leftvis/segmentation_results',
    'left_fullseg': '/home/jiarui/git/Deeplab_GTA/leftvis_fullseg/segmentation_results',
    'disparity': '/mnt/ngv/self-supervised-learning/Datasets/GTA_Dataset/disparity/0003',
}

_PATTERN_MAP = {
    'right': '_prediction.png',
    'right_fullseg': '_prediction.png',
    'left': '_prediction.png',
    'left_fullseg': '_prediction.png',
    'disparity': '.png',
}
...
def bilinear_sampler_1d_h(input_images, x_offset, depth_map, wrap_mode='edge', name='bilinear_sampler', **kwargs):
    ...
    def _transform(input_images, x_offset):
        with tf.variable_scope('transform'):
            ...
            #image_inpaint = cv.inpaint(image_output, image_mask, 3, cv.INPAINT_NS)
            #plt.figure()
            #plt.subplot(211)
...
def main(_):
    ...
    for i in range(num_images):
        right_image  = read_image(right_image_files[107])
        ...
        disparity_map  = tf.image.decode_png(tf.read_file(disparity_map_files[107]))
        disparity_map  = tf.image.convert_image_dtype(disparity_map,  tf.uint8)
        disparity_map = tf.cast(disparity_map, tf.float32)
        #disparity_map = tf.divide(disparity_map-1, 256)
        #disparity_map = tf.clip_by_value(disparity_map, 0.0,  2047.0)

        # Compute depth. The depth calculated is
        depth_map = tf.divide(1.0, disparity_map) # in meters
        ...
        #left_est = tf.cast(left_est, tf.uint8)
        ...
        image_name = os.path.basename(right_image_files[107])
        save_dir = '/home/jiarui/git/Deeplab_GTA/Results';
